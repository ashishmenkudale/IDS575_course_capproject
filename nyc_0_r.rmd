...
#https://stackoverflow.com/questions/31064353/how-to-make-a-figure-caption-in-rmarkdown
...
#
# ggplot objects can be passed in to plotlist (as a list of ggplot objects)
...
sample_submit <- as.tibble(fread('C:/Users/Dell/OneDrive/UIC/Fall_2017/IDS 575/IDS575_pro1/sample_submission.csv'))
```
...
```{r}
#summary of training dataset in representable format with pander.
pander(summary(train), caption = "Summary of training data", style = "grid")
```
...
```{r}
#summary of testing dataset in representable format with pander.
pander(summary(test), caption = "Summary of test data set", style = "grid")
```


```{r}
#see test data in a glance.
glimpse(test)
```
...
sum(is.na(train)) #sum all the datapoints in training dataset which have missing data.
sum(is.na(test)) #sum all the datapoints in testing dataset which have missing data.
```
...
```{r}
# create a combined dataframe of train and test data points named 'combine'.
# while doing that, create a new column which takes two values as 'train' and 'test'.
# If the datapoint was from train dataset, the the value of variable dset is 'train'. Else 'test'.
# For all the test datapoints, put 'NA' as value in columns 'dropoff_time' and 'trip_duration'.
# Using dplyr makes it easy.
...
train <- train %>%
  mutate(pickup_datetime = ymd_hms(pickup_datetime), #pickup_datetime as datetime in format ymd_hms
         dropoff_datetime = ymd_hms(dropoff_datetime), #dropoff_datetime as datetime in format ymd_hms
         vendor_id = factor(vendor_id), #vendor_id as factor
         passenger_count = factor(passenger_count)) #passanger_count id as factor

#ymd_hms => year month day_hour minute second
```
...
```{r}
# with dplyr,
# create new variable 'check'
# if the addition of difference between 'dropoff_datetime' and 'pickup_datetime' (-ve value) and positive 'trip_durations'
# is greater than zero, then date times are not consistent.
# Basically we are checkingif the 'trip_duration' is same as difference between 'dropoff_datetime' and 'pickup_datetime'.
# If it is incosistent, 'check takes value as 'true'.

# after creating 'check', subset train data with only 4 attributes as,
# 'check', 'pickup_datetime', 'dropoff_datetime', 'trip_duration'

# use group_by on this subset with variable 'check'

# report counts of check.
...
*check* does not take valule as *true* and we find that everything fits perfectly.
...
# All 2 million datapoints would make graph too much crowded.
#For representation purpose, randomly 8000 datapoints are selected and plotte.

# subsetting 8000 datapoints at random.
set.seed(1234)
foo <- sample_n(train, 8e3)

# Using subset With variables 'pickup_longitude and 'pickup_latitude' graphs are plotted.
...
```{r}
weather <- as.tibble(fread("C:/Users/Dell/OneDrive/UIC/Fall_2017/IDS 575/IDS575_pro1/weather_data_nyc_centralpark_2016.csv"))
```
...
foo <- as.tibble(fread("C:/Users/Dell/OneDrive/UIC/Fall_2017/IDS 575/IDS575_pro1/fastest_routes_train_part_1.csv"))
bar <- as.tibble(fread("C:/Users/Dell/OneDrive/UIC/Fall_2017/IDS 575/IDS575_pro1/fastest_routes_train_part_2.csv"))
foobar <- as.tibble(fread("C:/Users/Dell/OneDrive/UIC/Fall_2017/IDS 575/IDS575_pro1/fastest_routes_test.csv"))
...
train_group <- train %>%
  mutate(tgroup = case_when(train$trip_duration < 3e2 ~ "fast",
                            train$trip_duration >= 3e2 & train$trip_duration <= 1.6e3 ~ "mid",
                            train$trip_duration > 1.6e3 ~ "slow"))
